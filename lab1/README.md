...
Итого получаются функции $E(w) = \big(L(w), C(w), M(w)\big)$ и $$D(w) = 10L(w) + C(w) + M(w)$$. Эти функции доказывают завершимость TRS (например D(w) уменьшается хотя бы на 1 при применении любого правила и не может быть отрицательна).
...
в нормальных формах исходной TRS. Поэтому К-Б будет бесконечно добавлять правила
